...
function math.degrees2radians(degVal)
...
function isLuaFunc(funcVar)
    if funcVar ~= nil and type(funcVar) == 'function' then
        return true
    end
...
function FSM(t)
    ...
    for _,v in ipairs(t) do
        ...
        a[old][event] = {new_state = new, action = action}
    end
    return a
end

function UE4BreakRotator(Rotation)
    local PitchValue = 0.0
    local YawValue = 0.0
    local RollValue = 0.0
    PitchValue,YawValue,RollValue = UE4.UKismetMathLibrary.BreakRotator(Rotation,PitchValue,YawValue,RollValue)
    return {Pitch = PitchValue , Yaw = YawValue , Roll = RollValue }
end