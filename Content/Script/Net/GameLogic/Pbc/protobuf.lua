...
function _encode_type_meta:__index(key)
	local t, msg = c._env_type(P, self._CType, key)
	local func = assert(_writer[t],string.format("key:%s ctype:%s", key,self._CType))(msg)
	self[key] = func
	return func
end
...
function M.register_file(filename)
	...
	local buffer = UE4.FFileHelper.LoadContentFileAllBytesForLua(filename)
