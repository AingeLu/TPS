...
local function _load_module(service_path, shortname)
	...
	--local buffer = UE4.FFileHelper.LoadFileAllBytesForLua(realpath)
	local buffer = UE4.FFileHelper.LoadContentFileAllBytesForLua(realpath)

	local routine, err = load(buffer , realpath , "bt", message)
	assert(routine, err)()
...
function service.dispatch(type, msg, sz)
    ...
    if DEBUG_DUMP == 1 then
		if message.name ~= "server.s2c_ping_rsp" then
			dump(data, "server message : " .. message.name .."	" .. tostring(sz))
		elseif DEBUG_DISABLE_DUMP_PING == 1 then
			dump(data, "server message : " .. message.name .."	" .. tostring(sz))
		end
	end
...
function service.invoke(message_type, parm, func)
	...
	if DEBUG_DUMP == 1 then
		if "client.c2s_ping_req" ~= message.name then
			dump(message, "client message : " .. message.name)
		elseif DEBUG_DISABLE_DUMP_PING == 1 then
			dump(message, "client message : " .. message.name)
		end
	end
